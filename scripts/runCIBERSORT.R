...
#main function
runCIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=FALSE, absolute=FALSE, abs_method='sig.score', estimates_filepath=NULL){
    ...
    #removing rows where the gene counts are less than 50 for all samples put together based on
    #CIBERSORTx portal s
    row_sums<- rowSums(Y)
    Y <- Y[which(row_sums >50),]
    ...
    if(is.null(estimates_filepath)) {
      base_name <- tools::file_path_sans_ext(basename(mixture_file))
      estimates_filepath <- file.path(getwd(), paste0(base_name, "CIBERSORT_estimates.csv"))
    }
    #save results
    write.table(rbind(header,output), file=estimates_filepath, sep=",", row.names=F, col.names=F, quote=F)

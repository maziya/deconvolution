...
metadata24 = read.csv("metadata_bulk24.csv")
pattern <- paste0("^(", paste(cell_types, collapse = "|"), ")\\d{2}_count_matrix\\.csv$")
count24_files <- list.files(pattern = pattern)

for (file in count24_files) {
  counts <- read.csv(file, row.names = 1, check.names = FALSE)
  metadata_celltype <- metadata24 %>%
